...
def run_benchmark(benchmark_abs_path, query, engine, from_disk):
    ...
    cmd = os.path.join(cstore_home, "build/release/benchmark/benchmark_runner") + " " + benchmark_abs_path
    if verbose:
        print(cmd)
    # os.system(cmd)
...
def draw_disk():
    ...
    plt.savefig(os.path.join(cstore_home, 'plot/result_from_disk.png'))
    plt.clf()
...
def draw_page_cache():
    ...
    plt.xlabel("tpch query")
    plt.ylabel("execution time/s")
    plt.savefig(os.path.join(cstore_home, 'plot/result_from_page_cache.png'))
    plt.clf()
def main():
    global verbose
    global cstore_home
    cstore_home = os.environ.get('CSTORE_HOME')
    if cstore_home == None:
        print("You need to set $CSTORE_HOME first.")
        return
    os.makedirs(os.path.join(cstore_home, "plot"), exist_ok=True)
    parser = argparse.ArgumentParser(description='Process some integers.')
    parser.add_argument('--cstore', dest='cstore_path', default="", help='benchmark path of cstore')
    ...
    cstore_benchmark = args.cstore_path
    ...
    benchmark_list = []
    if cstore_benchmark != "":
        benchmark_list.append(["cstore", cstore_benchmark])
